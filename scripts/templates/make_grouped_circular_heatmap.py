...
def text_rotation(angle_deg: float) -> tuple[float, str]:
    ...
    while rotation < -90:
        rotation += 180
    return rotation, ("right" if 90 < normalized < 270 else "left")
...
def draw_group_labels(
    ...
    short_labels = [
        "Sleep → surface area",
        "Sleep → thickness",
        "Sleep → subcortical volume",
        "Sleep → longitudinal change",
        "Surface area → sleep",
        "Thickness → sleep",
        "Volume → sleep",
        "Change → sleep",
    ...
    cursor = 0
    for group, label in zip(PAIR_GROUPS, short_labels, strict=True):
        angle_deg = start_angle + (cursor + group.count / 2.0) * step_angle
...
def add_center_legend(fig: plt.Figure) -> None:
    left, bottom, width, height = 0.235, 0.405, 0.345, 0.225
    ax = fig.add_axes([left, bottom, width, height])
    ax.set_zorder(4)
    ax.axis("off")
    ax.text(0.00, 1.02, "Exposure and outcome pairs", fontsize=9.5, fontweight="bold", ha="left", va="bottom")
    ...
    handles = [
        Patch(facecolor=group.color, edgecolor="white", label=label)
        for group, label in zip(PAIR_GROUPS, short_labels, strict=True)
    ]
    ax.legend(
        handles=handles,
...
def make_figure(output_stem: Path) -> None:
    ...
    fig.text(0.395, 0.985, "Grouped circular heatmap", ha="center", va="top", fontsize=16, weight="bold", color="#222222")
    fig.text(0.395, 0.963, "Eight sleep-related traits across 92 exposure–outcome pairs", ha="center", va="top", fontsize=9.2, color="#5a5a5a")
    ...
    # The center key is the authoritative group label.  Omitting 92 tiny
    # outer annotations keeps the circular layout clean at publication size.
