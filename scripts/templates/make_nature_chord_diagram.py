...
def text_rotation(theta_deg: float) -> tuple[float, str]:
    ...
    while rotation < -90:
        rotation += 180
    return rotation, ("right" if 90 < angle < 270 else "left")
...
def build_flows(nodes: list[NodeSpec]) -> list[tuple[str, str, float]]:
    # A deterministic, data-like connectivity pattern.  The preview keeps a
    # restrained number of background links so the main relationships remain
    # legible; real dense data should be filtered to its top relationships.
    ...
    for _ in range(18):
        ...
        if len(high_weight_labels) and rng.random() < 0.55:
    ...
    return [flow for flow in flows if flow[0] in label_set and flow[1] in label_set]
...
def make_figure(output_stem: Path) -> None:
    ...
    # Show the largest sectors in the preview.  This is the same visual
    # grammar used for real data, but avoids a label wall in a generic demo.
    display_nodes = sorted(NODES, key=lambda node: node.weight, reverse=True)[:24]
    layout = compute_layout(display_nodes, start_angle=96.0, gap=1.15)
    flows = build_flows(display_nodes)
    color_lookup = {node.label: node.color for node in display_nodes}

    fig, ax = plt.subplots(figsize=(11.5, 11.0), facecolor="white")
    ax.set_aspect("equal")
    ax.axis("off")
    ax.set_xlim(-1.43, 1.43)
    ax.set_ylim(-1.40, 1.43)
    ax.text(0.0, 1.365, "Chord diagram", ha="center", va="bottom", fontsize=20, weight="bold", fontfamily="serif")
    ax.text(0.0, 1.315, "Top 24 functional domains · strongest relationships emphasized", ha="center", va="bottom", fontsize=9.5, color="#5a5a5a")
    ...
    draw_sector_ring(ax, layout, display_nodes)
    draw_labels(ax, layout, display_nodes)
