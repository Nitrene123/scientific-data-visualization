...
def draw_panel_a(ax: plt.Axes) -> None:
    ...
    header = Rectangle((0, -1.48), 375, 0.94, facecolor="#e8eef0", edgecolor="#7b8c91", linewidth=0.7, clip_on=False)
    ax.add_patch(header)
    ax.text(187.5, -1.02, "a  ·  Urban parks by city", ha="center", va="center", fontsize=10, weight="bold", color="#263438")
    ...
    ax.legend(handles=handles, title="Cooling effect", loc="lower right", bbox_to_anchor=(0.98, 0.01), fontsize=7.6, title_fontsize=8.5, frameon=True, framealpha=0.92)
...
def draw_vertical_boxplot_panel(ax: plt.Axes, metric: str, metric_data: list[np.ndarray]) -> None:
    ...
    tick_positions = [1, 5, 10, 15, 20, len(CITY_SPECS)]
    ax.set_xticks(tick_positions)
    ax.set_xticklabels([f"{i:02d}" for i in tick_positions], fontsize=7.5)
    if metric in {"PCI", "PCG"}:
        ax.set_xlabel("City ID", fontsize=8, labelpad=2)
    else:
        ax.set_xlabel("")
...
def add_city_and_legend_panel(ax: plt.Axes) -> None:
    ...
    ax.text(0.00, 0.98, "City-level summary", fontsize=10, fontweight="bold", color="#263438", va="top")
    ax.text(0.00, 0.91, "City IDs 01–26 follow panel a order; compact IDs keep panel c readable.", fontsize=7.6, color="#555555", va="top")

    y0 = 0.72
    ax.add_patch(Rectangle((0.00, y0), 0.09, 0.035, facecolor="white", edgecolor="#333333", linewidth=0.7))
    ax.text(0.12, y0 + 0.017, "25–75% interval", va="center", fontsize=7.7)
    ax.plot([0.00, 0.09], [y0 - 0.050, y0 - 0.050], color="#315f5e", lw=2.0)
    ax.text(0.12, y0 - 0.050, "Median", va="center", fontsize=7.7)
    ax.scatter(0.045, y0 - 0.105, marker="^", s=16, color="#d44d5d", edgecolor="white", linewidth=0.3)
    ax.text(0.12, y0 - 0.105, "Mean", va="center", fontsize=7.7)
    ...
    ax.text(0.12, y0 - 0.160, "Range (1.5×IQR)", va="center", fontsize=7.7)
    ax.plot([0.00, 0.09], [y0 - 0.215, y0 - 0.215], color="#2f6791", lw=0.9)
    ax.text(0.12, y0 - 0.215, "Mean trend across cities", va="center", fontsize=7.7)

    for idx, group in enumerate(GROUP_ORDER):
        y = y0 - idx * 0.065
        ax.add_patch(Rectangle((0.57, y), 0.09, 0.035, facecolor=GROUP_COLORS[group], edgecolor="white", linewidth=0.5))
        ax.text(0.69, y + 0.017, group, va="center", fontsize=7.3)
...
def make_figure(output_stem: Path) -> None:
    ...
    fig.text(0.5, 0.985, "Urban park cooling effects across 26 cities", ha="center", va="top", fontsize=15, weight="bold", color="#222222")
